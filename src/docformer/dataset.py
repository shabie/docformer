...
pathToSave = 'RVL-CDIP-PickleFiles/'
...
def createPickelFile(image,label,pathToSave,tokenizer=None,target_size = 224,max_seq_length = 512,pretrain = True):
...
# output = list(map(lambda x:createPickelFile(x[0],x[1],pathToSave),list(zip(images,labels))))

# items = os.listdir(pathToSave)
# items

# imageName = images[0].split('/')[-1]
# with open(os.path.join(pathToSave,imageName+'.pickle'),'rb') as f:
