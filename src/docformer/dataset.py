...
# path_to_save = "RVL-CDIP-PickleFiles/"

# label2idx = {}
# idx2label = []
# images = []
# labels = []
# basePath = "RVL-CDIP/train/"
...
def relative_position_bucket(
        relative_position, bidirectional=True, num_buckets=32, max_distance=1000
):
    """
...
def apply_ocr(example):
    ...
    ocr_df = ocr_df.dropna().reset_index(drop=True)
    ocr_df = ocr_df.sort_values(
        by=["left", "top"]
    )  # Sorting the values, on the basis of the coordinate space
...
def get_tokens(tokenizer, words, unnormalized_word_boxes, normalized_word_boxes):
    ...
    for word, unnormalized_box, box in zip(
            words, unnormalized_word_boxes, normalized_word_boxes
    ):
...
def get_relative_distance(actual_bbox, centroid, index):
    ...
    def func(coordinates):
        return relative_position_bucket(
            torch.as_tensor(coordinates).type(torch.IntTensor)
        ).tolist()
...
def create_pickle_file(
    ...
    token_boxes, unnormalized_token_boxes, final_word_tokens = get_tokens(
    ...
    unnormalized_token_boxes = (
            [[0, 0, 0, 0]] + unnormalized_token_boxes + [[0, 0, 0, 0]]
    )
    ...
    encoding['resized_image_super_resolution'] = ToTensor()(resized_image_super_resolution)/255.0       # Normalizing the image
    ...
    encoding["resized_and_aligned_bounding_boxes"] = [
        resize_and_align_bounding_box(bbox, original_image, target_size)
