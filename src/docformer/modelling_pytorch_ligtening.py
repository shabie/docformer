...
class RVLCDIPDatset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = self.image_file_names[index]
        label = self.imageClass[index]
        imageName = image.split('/')[-1]
        encoding = os.path.join(self.pathToPickleFile,imageName)
        encoding+='.pickle'                                             # Address of the pickle file, which was generated from the dataset.py
...
class DataModule(pl.LightningDataModule):
      def __init__(self, batch_size = 32,train_image_files,train_image_Class,val_image_files,val_image_Class):
          super().__init__()
          self.batch_size = batch_size
          self.train_image_files = train_image_files
          self.train_image_Class = train_image_Class
          self.val_image_files = train_image_files
          self.val_image_Class = train_image_Class

      def setup(self,stage = None):
        self.train_dataset = RVLCDIPDatset(train_image_files,train_image_Class,pathToPickleFile)
        self.val_dataset  =  RVLCDIPDatset(val_image_files,val_image_Class,pathToPickleFile)
